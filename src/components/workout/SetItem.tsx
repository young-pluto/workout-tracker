...
export function SetItem({
  ...
  // Handle blur - auto-save if valid data, but NOT when focus just moved
  // to another control within this same set (e.g. reps -> RPE -> remarks).
  ...
  // RPE display state
  const hasRpe = set.rpe !== '';
  const rpeVal = hasRpe ? Number(set.rpe) : 6;
  const rpePct = ((rpeVal - 6) / 4) * 100;
  const rpeFill = hasRpe ? rpeColor(rpeVal) : 'var(--color-text-muted)';
  ...
  if (set.isSaved) {
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 flex items-center gap-2 text-[14px] min-w-0">
          ...
          {set.rpe && (
            <span
              ...
              style={{ backgroundColor: `${rpeColor(Number(set.rpe))}22`, color: rpeColor(Number(set.rpe)) }}
            >
              RPE {set.rpe}
            </span>
          )}
        ...
        {/* Edit indicator */}
        <svg className="w-4 h-4 text-[var(--color-text-muted)] dark:text-[var(--color-dark-text-muted)] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={onSave}
          className="p-2 rounded-lg bg-[var(--color-success)] text-white active:scale-95 transition-transform shrink-0"
        >
        ...
        {canDelete && (
          <button
            onClick={onDelete}
            className="p-2 rounded-lg text-[var(--color-text-muted)] dark:text-[var(--color-dark-text-muted)] active:text-[var(--color-error)] active:bg-[var(--color-error-bg)] transition-colors shrink-0"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Row 2: RPE slider */}
      <div className="flex items-center gap-3 pl-7 pr-1">
        <span className="text-[11px] font-medium uppercase tracking-wider text-[var(--color-text-muted)] dark:text-[var(--color-dark-text-muted)] w-8 shrink-0">
          RPE
        </span>
        <input
          type="range"
          ...
          onBlur={handleBlur}
          className="rpe-range flex-1"
          style={{ ['--rpe-pct' as string]: `${rpePct}%`, ['--rpe-fill' as string]: rpeFill }}
          aria-label={`RPE for set ${setNumber}`}
        />
        <span
          className="w-9 text-center text-[13px] font-bold tabular-nums shrink-0"
          style={{ color: hasRpe ? rpeFill : 'var(--color-text-muted)' }}
        >
          {hasRpe ? set.rpe : '—'}
        </span>
      </div>
      ...
      <div className="pl-7 pr-1">
        <input
          ...
          className="w-full px-3 py-2 text-[14px] bg-[var(--color-background)] dark:bg-[var(--color-dark-background)] border border-[var(--color-border)] dark:border-[var(--color-dark-border)] rounded-lg text-[var(--color-text-primary)] dark:text-[var(--color-dark-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] dark:focus:ring-[var(--color-dark-accent)]"
        />
      </div>
    </motion.div>
  );
